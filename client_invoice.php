...
if(!isset($_SESSION['iwcid'])) {
	...
	$secresult = $sql_command->count_rows("$database_customer_invoices,$database_order_details,$database_clients","$database_clients.iwcuid","$database_clients.iwcuid='".$_SESSION['iwcid']."' AND $database_customer_invoices.id='".addslashes($_GET["invoice"])."' AND $database_customer_invoices.order_id=$database_order_details.id AND $database_order_details.client_id=$database_clients.id");
