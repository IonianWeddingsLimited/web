...
if (empty($currency_name)) {
	$result = $sql_command->select("quotation_proformas,
								   quotation_details,
								   $database_clients",
								   "quotation_proformas.order_id,
								   quotation_proformas.exchange_rate",
								   "WHERE quotation_proformas.id='" . $invoiceno . "' 
								   AND quotation_proformas.order_id=quotation_details.id 
								   AND quotation_details.client_id=$database_clients.id");
	$invrecord = $sql_command->result($result);

	$invoice_result = $sql_command->select("quotation_proforma_history","qty,
											 cost,
											 iw_cost,
											 discount_percent,
											 d_value,
	...
	if ($currency_name=="Pound") {
		...
		$invoice_row = $sql_command->results($invoice_result);
			$poundone = "select qty,
											 ...
											 d_value,
											 d_type,
											 d_ from quotation_proforma_history WHERE invoice_id='".addslashes($invoiceno)."' 
											 and order_id='".addslashes($invrecord[0])."' 
											 and currency='Pound' and item_type='Package'";
		$show_additional = "No";
		$total_payment = 0;
	...
	if ($currency_name=="Pound") {
		...
		$invoice_result = $sql_command->select("quotation_proforma_history","qty,
			...
			$poundtwo = "select qty,
											 ...
											 d_value,
											 d_type,
											 d_ FROM quotation_proforma_history WHERE invoice_id='".addslashes($invoiceno)."' and order_id='".addslashes($invrecord[0])."' and currency='Pound' and item_type!='Package'";
			foreach($invoice_row as $invoice_record) {
				$show_additional = "Yes";
			}
			$show_additional = "No";
