...
header("Location: $site_url/included-in-package.php?a=island&island_id=".$_POST["island_id"]);
exit();
}
...
if(!$_POST["package_id"]) {
$get_template->topHTML();
$get_template->errorHTML("Select Package","Oops!","Please select a package","Link","included-in-package.php?a=island&island_id=".addslashes($_POST["island_id"])."");
$get_template->bottomHTML();
$sql_command->close();
}
...
<form action="<?php echo $site_url; ?>/included-in-package.php" method="POST">
