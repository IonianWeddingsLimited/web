...
$sql_command->update($database_packages,"ceremony_id='".addslashes($ceremony)."'","island_id='".addslashes($_POST["island_id"])."' and id='".addslashes($_POST["package_id"])."'");
$sql_command->update($database_packages,"reception_id='".addslashes($venue)."'","island_id='".addslashes($_POST["island_id"])."' and id='".addslashes($_POST["package_id"])."'");
...
$cols = "package_id,iw_name,cost,iw_cost,currency,type,notes,timestamp,deleted";
...
if($menu_record[4] == $receiption_record[0]) { $selected = "selected=\"selected\"";  } else { $selected = "";  }
...
<div style="clear:left;"></div>
<div style="float:left; width:160px; margin:5px;"><b>Ceremony/Reception Location</b><p>Please select a rerception name (black writing)</p></div>
