...
$list_years .= "<option value=\"".$current_year."\">".$current_year."</option>";	
$current_year++;
}
...
$april_result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_package_info,$database_packages","$database_customer_invoices.cost,
									 $database_customer_invoices.iw_cost,
									 $database_customer_invoices.vat","WHERE $database_customer_invoices.updated_timestamp > $april_start and 
									 $database_customer_invoices.updated_timestamp < $may_start and 
									 $database_customer_invoices.status='Paid' and 
...
$april_iwcost += $april_record[1];
$april_vat += ($april_record[1] / 100) * $april_record[2];
}
$april_profit = $april_iwcost - $april_cost;


$may_result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_package_info,$database_packages","$database_customer_invoices.cost,
									 $database_customer_invoices.iw_cost,
									 $database_customer_invoices.vat","WHERE $database_customer_invoices.updated_timestamp > $may_start and 
									 $database_customer_invoices.updated_timestamp < $june_start and 
									 $database_customer_invoices.status='Paid' and 
									 $database_customer_invoices.order_id=$database_order_details.id and 
									 $database_order_details.package_id=$database_package_info.id and 
									 $database_package_info.package_id=$database_packages.id and 
...
$may_iwcost += $may_record[1];
$may_vat += ($may_record[1] / 100) * $may_record[2];
}
$may_profit = $may_iwcost - $may_cost;


$june_result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_package_info,$database_packages","$database_customer_invoices.cost,
									 $database_customer_invoices.iw_cost,
									 $database_customer_invoices.vat","WHERE $database_customer_invoices.updated_timestamp > $june_start and 
									 $database_customer_invoices.updated_timestamp < $july_start and 
									 $database_customer_invoices.status='Paid' and 
									 $database_customer_invoices.order_id=$database_order_details.id and 
									 $database_order_details.package_id=$database_package_info.id and 
									 $database_package_info.package_id=$database_packages.id and 
...
$june_iwcost += $june_record[1];
$june_vat += ($june_record[1] / 100) * $june_record[2];
}
$june_profit = $june_iwcost - $june_cost;


$july_result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_package_info,$database_packages","$database_customer_invoices.cost,
									 $database_customer_invoices.iw_cost,
									 $database_customer_invoices.vat","WHERE $database_customer_invoices.updated_timestamp > $july_start and 
									 $database_customer_invoices.updated_timestamp < $august_start and 
									 $database_customer_invoices.status='Paid' and 
									 $database_customer_invoices.order_id=$database_order_details.id and 
									 $database_order_details.package_id=$database_package_info.id and 
									 $database_package_info.package_id=$database_packages.id and 
...
$july_iwcost += $july_record[1];
$july_vat += ($july_record[1] / 100) * $july_record[2];
}
$july_profit = $july_iwcost - $july_cost;


$august_result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_package_info,$database_packages","$database_customer_invoices.cost,
									 $database_customer_invoices.iw_cost,
									 $database_customer_invoices.vat","WHERE $database_customer_invoices.updated_timestamp > $august_start and 
									 $database_customer_invoices.updated_timestamp < $september_start and 
									 $database_customer_invoices.status='Paid' and 
									 $database_customer_invoices.order_id=$database_order_details.id and 
									 $database_order_details.package_id=$database_package_info.id and 
									 $database_package_info.package_id=$database_packages.id and 
...
$august_iwcost += $august_record[1];
$august_vat += ($august_record[1] / 100) * $august_record[2];
}
$august_profit = $august_iwcost - $august_cost;



$september_result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_package_info,$database_packages","$database_customer_invoices.cost,
									 $database_customer_invoices.iw_cost,
									 $database_customer_invoices.vat","WHERE $database_customer_invoices.updated_timestamp > $september_start and 
									 $database_customer_invoices.updated_timestamp < $october_start and 
									 $database_customer_invoices.status='Paid' and 
									 $database_customer_invoices.order_id=$database_order_details.id and 
									 $database_order_details.package_id=$database_package_info.id and 
									 $database_package_info.package_id=$database_packages.id and 
...
$september_iwcost += $september_record[1];
$september_vat += ($september_record[1] / 100) * $september_record[2];
}
$september_profit = $september_iwcost - $september_cost;



$october_result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_package_info,$database_packages","$database_customer_invoices.cost,
									 $database_customer_invoices.iw_cost,
									 $database_customer_invoices.vat","WHERE $database_customer_invoices.updated_timestamp > $october_start and 
									 $database_customer_invoices.updated_timestamp < $november_start and 
									 $database_customer_invoices.status='Paid' and 
									 $database_customer_invoices.order_id=$database_order_details.id and 
									 $database_order_details.package_id=$database_package_info.id and 
									 $database_package_info.package_id=$database_packages.id and 
...
$october_iwcost += $october_record[1];
$october_vat += ($october_record[1] / 100) * $october_record[2];
}
$october_profit = $october_iwcost - $october_cost;



$november_result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_package_info,$database_packages","$database_customer_invoices.cost,
									 $database_customer_invoices.iw_cost,
									 $database_customer_invoices.vat","WHERE $database_customer_invoices.updated_timestamp > $november_start and 
									 $database_customer_invoices.updated_timestamp < $december_start and 
									 $database_customer_invoices.status='Paid' and 
									 $database_customer_invoices.order_id=$database_order_details.id and 
									 $database_order_details.package_id=$database_package_info.id and 
									 $database_package_info.package_id=$database_packages.id and 
...
$november_iwcost += $november_record[1];
$november_vat += ($november_record[1] / 100) * $november_record[2];
}
$november_profit = $november_iwcost - $november_cost;



$december_result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_package_info,$database_packages","$database_customer_invoices.cost,
									 $database_customer_invoices.iw_cost,
									 $database_customer_invoices.vat","WHERE $database_customer_invoices.updated_timestamp > $december_start and 
									 $database_customer_invoices.updated_timestamp < $january_start and 
									 $database_customer_invoices.status='Paid' and 
									 $database_customer_invoices.order_id=$database_order_details.id and 
									 $database_order_details.package_id=$database_package_info.id and 
									 $database_package_info.package_id=$database_packages.id and 
...
$december_iwcost += $december_record[1];
$december_vat += ($december_record[1] / 100) * $december_record[2];
}
$december_profit = $december_iwcost - $december_cost;



$january_result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_package_info,$database_packages","$database_customer_invoices.cost,
									 $database_customer_invoices.iw_cost,
									 $database_customer_invoices.vat","WHERE $database_customer_invoices.updated_timestamp > $january_start and 
									 $database_customer_invoices.updated_timestamp < $february_start and 
									 $database_customer_invoices.status='Paid' and 
									 $database_customer_invoices.order_id=$database_order_details.id and 
									 $database_order_details.package_id=$database_package_info.id and 
									 $database_package_info.package_id=$database_packages.id and 
...
$january_iwcost += $january_record[1];
$january_vat += ($january_record[1] / 100) * $january_record[2];
}
$january_profit = $january_iwcost - $january_cost;


$february_result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_package_info,$database_packages","$database_customer_invoices.cost,
									 $database_customer_invoices.iw_cost,
									 $database_customer_invoices.vat","WHERE $database_customer_invoices.updated_timestamp > $february_start and 
									 $database_customer_invoices.updated_timestamp < $march_start and 
									 $database_customer_invoices.status='Paid' and 
									 $database_customer_invoices.order_id=$database_order_details.id and 
									 $database_order_details.package_id=$database_package_info.id and 
									 $database_package_info.package_id=$database_packages.id and 
...
$february_iwcost += $february_record[1];
$february_vat += ($february_record[1] / 100) * $february_record[2];
}
$february_profit = $february_iwcost - $february_cost;


$march_result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_package_info,$database_packages","$database_customer_invoices.cost,
									 $database_customer_invoices.iw_cost,
									 $database_customer_invoices.vat","WHERE $database_customer_invoices.updated_timestamp > $march_start and 
									 $database_customer_invoices.updated_timestamp < $april2_start and 
									 $database_customer_invoices.status='Paid' and 
									 $database_customer_invoices.order_id=$database_order_details.id and 
									 $database_order_details.package_id=$database_package_info.id and 
									 $database_package_info.package_id=$database_packages.id and 
...
$march_iwcost += $march_record[1];
$march_vat += ($march_record[1] / 100) * $march_record[2];
}
...
?>
<h1>Destination Billing Report</h1>
