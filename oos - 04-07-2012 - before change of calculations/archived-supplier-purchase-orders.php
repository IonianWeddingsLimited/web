...
$result = $sql_command->select("$database_supplier_invoices,$database_supplier_details,$database_order_details,$database_clients","$database_supplier_invoices.id,
							   ...
							   ($database_supplier_invoices.status != 'Outstanding' and $database_supplier_invoices.status != 'Pending') and 
...
<div style=\"float:left; width:60px; margin:5px;\"><a href=\"$site_url/oos/purchase-order.php?purchase_order=".$record[0]."\">$p_curreny ".$record[2]."</a></div>
