...
$result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_clients","$database_clients.*,
							   ...
							   $database_order_details.exchange_rate,
...
$pdf->Cell(70,5,"Receipt",'',0,'R',true);
...
$invoice_result = $sql_command->select($database_invoice_history,"name,
									   ...
									   code,
									   d_,
									   item_type","WHERE invoice_id='".addslashes($_GET["invoice"])."' and order_id='".addslashes($record[20])."' and currency='Euro'");
...
} elseif($invoice_record[11] == "Percent" and $invoice_record[10] >= 1) { 
...
$iw_cost = $iw_cost - abs($invoice_record[11] / $invoice_record[3]);
} 


$payment_total_orig = $invoice_record[1] * $invoice_record[3];			
...
$invoice_result = $sql_command->select($database_invoice_history,"name,
									   ...
									   code,
									   d_,
									   item_type","WHERE invoice_id='".addslashes($_GET["invoice"])."' and order_id='".addslashes($record[20])."' and currency='Pound'");
$invoice_row = $sql_command->results($invoice_result);
...
$invoice_result = $sql_command->select($database_invoice_history,"name,
									   ...
									   iw_cost,
									   currency,
									   timestamp","WHERE order_id='".addslashes($record[20])."' and item_type='Deposit' and status='Paid'");
$invoice_record = $sql_command->result($invoice_result);

if($invoice_record[0]) {
if($record[22] < 1) { $record[22] = 1; }	
if($invoice_record[4] == "Pound") {
$total_deposit_paid = $invoice_record[3];
} else {
$total_deposit_paid = $invoice_record[3] / $record[22];
}
