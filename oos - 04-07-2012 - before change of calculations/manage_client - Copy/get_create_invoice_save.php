...
$sql_command->insert($database_customer_invoices,$cols,"'".addslashes($_POST["invoice_id"])."','','','20.00','Pending','$time','Order',''");
...
if($order_record[17] == "Amount" and $order_record[16] >= 1) { 
...
$total_iw_cost = $order_record[4] * $iw_cost;
...
$deposit_order_result = $sql_command->select($database_order_history,"iw_cost,currency,exchange_rate","WHERE order_id='".addslashes($_POST["invoice_id"])."' and item_type='Deposit' and (status='Paid' or status='Pending' or status='Outstanding')");
$deposit_order_record = $sql_command->result($deposit_order_result);


if($deposit_order_record[0]) {
if($deposit_order_record[1] == "Pound") {
$workout_deposit = $deposit_order_record[0] * $deposit_order_record[2];
$order_record[6] = $order_record[6] - $workout_deposit;
} else {
$workout_deposit = $deposit_order_record[0];
$order_record[6] = $order_record[6] - $workout_deposit;
}
$total_due_package -= $workout_deposit;
}
}
