...
<form action="<?php echo $site_url; ?>/admin/update-press.php" method="POST" name="news">
<input type="hidden" name="id" value="<?php echo stripslashes($record[0]); ?>" />
<div style="float:left; width:160px; margin:5px;"><b>News Date</b></div>
...
<form action="<?php echo $site_url; ?>/admin/update-press.php" method="POST" name="testimonial">
<input type="hidden" name="id" value="<?php echo stripslashes($record[0]); ?>" />
...
if($error) {
$get_template->topHTML();
$get_template->errorHTML("Update Press Info","Oops!","$error","Link","admin/update-press.php");
$get_template->bottomHTML();
$sql_command->close();
}
...
$get_template->topHTML();
?>
<h1>Press Info Updated</h1>

<p>The press info has now been updated</p>
<?
$get_template->bottomHTML();
$sql_command->close();
...
$get_template->topHTML();
?>
<h1>Press Info Deleted</h1>

<p>The press info has now been deleted</p>
<?
$get_template->bottomHTML();
$sql_command->close();	
...
$result = $sql_command->select($database_news,"id,title,timestamp","WHERE type='Press' ORDER BY timestamp DESC");
...
<form action="<?php echo $site_url; ?>/admin/update-press.php" method="POST">
