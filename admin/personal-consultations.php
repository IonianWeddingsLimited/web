...
if(!$_GET["id"]) {
header("Location: $site_url/admin/personal-consultations.php");
$sql_command->close();
}
...
<div>
<form action="<?php echo $site_url; ?>/admin/personal-consultations.php" method="POST" class="pageform">
...
<form action="<?php echo $site_url; ?>/admin/personal-consultations.php" method="POST">
<input type="hidden" name="id" value="<?php echo stripslashes($record[0]); ?>" />
...
$sql_command->delete($database_info_personal_consultations,"id='".addslashes($_POST["id"])."'");
...
if($_GET["surname"]) {
$result = $sql_command->select($database_info_personal_consultations,"id,firstname,lastname,archive,timestamp,ip","WHERE lastname like '".addslashes($_GET["surname"])."%' ORDER BY lastname");
$row = $sql_command->results($result);
} else {
$result = $sql_command->select($database_info_personal_consultations,"id,firstname,lastname,archive,timestamp,ip","ORDER BY timestamp DESC");
$row = $sql_command->results($result);
}
...
$get_ip_results = $sql_command->select($database_info_personal_consultations,"ip,count(id)","WHERE ip!='' GROUP BY ip");
...
?>
<h1>Form - Personal Consultations</h1>
<form action="<?php echo $site_url; ?>/admin/personal-consultations.php" method="get">
