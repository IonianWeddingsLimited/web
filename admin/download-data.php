...
fputcsv($output, array('#', 'Enquiry date', 'Brides Name', 'Groom Name', 'Bride Email', 'Bride Telephone', 'Groom Email', 'Groom Telephone', 'Island', 'Wedding date', 'Source', 'Budget', 'Comments', 'Callback', 'Residence'));
...
$result = $sql_command->select("$database_info_wedding_questionnaire$add_des_line","$database_info_wedding_questionnaire.id,
							   ...
							   $database_info_wedding_questionnaire.groom_email",$where_line."  GROUP BY $database_info_wedding_questionnaire.id ORDER BY $database_info_wedding_questionnaire.id DESC");
...
fputcsv($output, array($record[0], $date2, $couple_names, $couple_names2, $record[4], $record[16], $record[18], $record[17], $island_html, $date, stripslashes($record[9]), stripslashes($record[10]), stripslashes($record[11]), $callback, stripslashes($record[3])));	
}
