...
if(!$_GET["id"]) {
header("Location: $site_url/admin/wedding-questionnaire.php");
$sql_command->close();
}
...
<div>
<form action="<?php echo $site_url; ?>/admin/wedding-questionnaire.php" method="POST" class="pageform">
...
<?php echo stripslashes($record[22]); ?>
	...
	<div class="formheader" style="width:600px;">
        ...
        <p style="font-size:11px;"><?php echo stripslashes($record[25]); ?></p>
        </div>
...
<form action="<?php echo $site_url; ?>/admin/wedding-questionnaire.php" method="POST">
<input type="hidden" name="id" value="<?php echo stripslashes($record[0]); ?>" />
...
$sql_command->delete($database_info_wedding_questionnaire,"id='".addslashes($_POST["id"])."'");
	
$get_template->topHTML();
?>
<h1>Questionnaire Deleted</h1>

<p>The questionnaire has now been deleted</p>
<?
$get_template->bottomHTML();
$sql_command->close();	
...
header("Location: ".$site_url."/admin/download-data.php?type=csv&data=".$_POST["data"]."&from=".$_POST["date_from"]."&to=".$_POST["date_to"]."&destination=".$_POST["destination_id"]);
exit();
}
...
<form action="<?php echo $site_url; ?>/admin/wedding-questionnaire.php" method="get">
