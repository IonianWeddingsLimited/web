...
if($_POST["action"] == "Delete Supplier")  {

if(!$_POST["id"]) { $error .= "Missing Supplier Id<br>"; }
	
	
	
if($error) {
$get_template->topHTML();
$get_template->errorHTML("Delete Supplier","Oops!","$error","Link","oos/update-supplier.php");
$get_template->bottomHTML();
$sql_command->close();
}

$sql_command->update($database_supplier_details,"deleted='Yes'","id='".addslashes($_POST["id"])."'");
$sql_command->insert($database_client_historyinfo,"client_id,user_id,comment,timestamp","'','".$login_record[1]."','Supplier Deleted (".addslashes($_POST["companyname"]).")','".$time."'");

$get_template->topHTML();
?>
<h1>Supplier Deleted</h1>

<p>The supplier has now been deleted</p>
...
$sql_command->insert($database_client_historyinfo,"client_id,user_id,comment,timestamp","'','".$login_record[1]."','Supplier Updated (".addslashes($_POST["companyname"]).")','".$time."'");
...
<div style="float:left; margin:5px;"><?php echo stripslashes($record[6]); ?><input type="hidden" name="companyname" value="<?php echo stripslashes($record[6]); ?>"/></div>
<div style="clear:left;"></div>
...
<div style="float:left; margin-left:240px; margin-top:10px; margin-right:10px;"><input type="button" name="" value="Back"  onclick="window.location='<?php echo $site_url; ?>/oos/update-supplier.php'"></div>
<div style="float:right; margin-top:10px; margin-right:10px;"><input type="submit" name="action" value="Delete Supplier" onclick="return deletechecked();"/></div>
