...
if(!eregi('package-extras', $one_newname)) { $error .= "Please upload package extras csv file<br>"; }

if($error) {
$get_template->topHTML();
$get_template->errorHTML("CSV Update - Package Extras","Oops!","$error","Link","oos/csv-package-extras.php");
...
$display_net = $array[5];
$display_gross = $array[6];

$array[5] = ereg_replace("[^0-9\.-]", "", $array[5]);
$array[6] = ereg_replace("[^0-9\.-]", "", $array[6]);

if($array[0] and $array[1] and $array[2] and $array[3] and $array[4] and $array[5] and $array[6] and $array[0] != "Record") {
...
<div style=\"float:left; width:420px;\">$array[4]</div>
...
<p>The package extras database has now been updated</p>

<p><input type="button" name="" value="Back" onclick="window.location='<?php echo $site_url; ?>/oos/csv-package-extras.php'"></p>
<hr />
<?php echo $html; ?>
<?
$get_template->bottomHTML();
...
$get_template->topHTML();
$get_template->errorHTML("CSV Update - Package Extras","Oops!","Please select an island","Link","oos/csv-package-extras.php");
$get_template->bottomHTML();
$sql_command->close();
}
...
$csv_string .= '"Record","Island","Category","Code","Extra Name","Net","Gross"'; 	
$csv_string .= "\n";
...
$csv_string .= '"' .$record[0]. '","' .$record[6] . '","' .$record[5]. '","' .stripslashes($record[4]). '","' .stripslashes($record[1]). '","'.$type.' ' .$record[2]. '","'.$type.' ' .$record[3].  '"'; 	
$csv_string .= "\n";
}
...
$filename = stripslashes($page_record[0])."-package-extras-".date('M-j-Y-G-i').".csv";
...
$get_template->topHTML();
?>
<h1>CSV Update - Package Extras</h1>

<h3>Download Prices</h3>
<form method="post" action="csv-package-extras.php">
...
<form method="post" action="csv-package-extras.php" enctype="multipart/form-data">
