...
$get_template->errorHTML("Manage Prospect","Oops!","$error","Link","oos/manage-prospect.php?a=deposit&id=".$_POST["client_id"]."&invoice_id=".$_POST["invoice_id"]);
...
$sql_command->insert(quotation_proformas,$cols,"'".addslashes($_POST["invoice_id"])."','','".addslashes($total_due)."','20.00','Pending','$time','Deposit','".$time."','".addslashes($_POST["exchange_rate"])."'");
$maxid = $sql_command->maxid("quotation_proformas","id","");

$sql_command->update("quotation_history","iw_cost='".addslashes($_POST["deposit"])."'","order_id='".addslashes($_POST["invoice_id"])."' and item_type='Deposit'");
$sql_command->update("quotation_history","currency='".addslashes($_POST["currency"])."'","order_id='".addslashes($_POST["invoice_id"])."' and item_type='Deposit'");
$sql_command->update("quotation_history","exchange_rate='".addslashes($_POST["exchange_rate"])."'","order_id='".addslashes($_POST["invoice_id"])."' and item_type='Deposit'");
$sql_command->update("quotation_history","status='Pending'","order_id='".addslashes($_POST["invoice_id"])."' and item_type='Deposit'");
...
$maxid = $sql_command->maxid("quotation_proforma_history","id","");
