...
$invoice_result = $sql_command->select($database_invoice_history,"name,
									   ...
									   d_value,
									   d_type,
									   code","WHERE invoice_id='".addslashes($_GET["invoice"])."' and order_id='".addslashes($record[20])."' and currency='Euro'");
...
$deposit_result = $sql_command->select($database_invoice_history,"iw_cost,status","WHERE order_id='".addslashes($record[20])."' and item_type='Deposit'");
$deposit_record = $sql_command->result($deposit_result);

if($record[22] < 1) { $record[22] = 1; }

$deposit_value = $deposit_record[0] * $record[22];
...
$outstanding_euros = $total_payment_euro - $deposit_value;
...
$invoice_result = $sql_command->select($database_invoice_history,"name,
									   ...
									   d_value,
									   d_type,
									   code","WHERE invoice_id='".addslashes($_GET["invoice"])."' and order_id='".addslashes($record[20])."' and currency='Pound'");
$invoice_row = $sql_command->results($invoice_result);
...
$total_gbp = $total_payment_euro + $total_payment_pound;
