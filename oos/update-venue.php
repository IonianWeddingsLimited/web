...
if($_POST["action"] == "Delete Venue")  {

if(!$_POST["id"]) { $error .= "Missing Venue Id<br>"; }
	
	
	
if($error) {
$get_template->topHTML();
$get_template->errorHTML("Delete Venue","Oops!","$error","Link","oos/update-venue.php");
$get_template->bottomHTML();
$sql_command->close();
}

$sql_command->update($database_venue_names,"deleted='Yes'","id='".addslashes($_POST["id"])."'");
$sql_command->insert($database_client_historyinfo,"client_id,user_id,comment,timestamp","'','".$login_record[1]."','Venue Deleted (".addslashes($_POST["name"]).")','".$time."'");

$get_template->topHTML();
?>
<h1>Venue Deleted</h1>

<p>The venue has now been deleted</p>
...
$sql_command->update($database_venue_names,"venue_name='".addslashes($_POST["name"])."', notes ='".addslashes($_POST["venue_note"])."'","id='".addslashes($_POST["id"])."'");
$sql_command->insert($database_client_historyinfo,"client_id,user_id,comment,timestamp","'','".$login_record[1]."','Venue Updated (".addslashes($_POST["name"]).")','".$time."'");
...
$result = $sql_command->select($database_venue_names,"id,island_id,venue_name,notes","WHERE id='".addslashes($_POST["id"])."'");
...
<div style="float:left; width:160px; margin:5px;"><b>Venue Note</b></div>
<div style="float:left; margin:5px;">
<textarea name="venue_note" id="venue_note" class="the_editor_min"><?php echo stripslashes($record[3]); ?></textarea><script type="text/javascript">
				CKEDITOR.replace( 'venue_note',
					{
						skin : 'kama',
						toolbar : 'PDF',
						width: 700,
...
<div style="float:left; margin-left:240px; margin-top:10px; margin-right:10px;"><input type="button" name="" value="Back" onclick="window.location='<?php echo $site_url; ?>/oos/update-venue.php'"></div>
<div style="float:right; margin-top:10px; margin-right:10px;"><input type="submit" name="action" value="Delete Venue" onclick="return deletechecked();"/></div>
...
$result = $sql_command->select($database_venue_names,"id,venue_name","WHERE island_id='".addslashes($_GET["island_id"])."' and deleted='No' ORDER BY venue_name");
