...
if(!$_POST["id"]) { $error .= "Missing Category Id<br>"; }
	
	
	
if($error) {
$get_template->topHTML();
$get_template->errorHTML("Delete Category","Oops!","$error","Link","oos/update-category.php");
$get_template->bottomHTML();
$sql_command->close();
}

$sql_command->update($database_category_extras,"deleted='Yes'","id='".addslashes($_POST["id"])."'");
$sql_command->insert($database_client_historyinfo,"client_id,user_id,comment,timestamp","'','".$login_record[1]."','Delete Category (".addslashes($_POST["name"]).")','".$time."'");

$get_template->topHTML();
?>
<h1>Category Deleted</h1>

<p>The category has now been deleted</p>
...
$sql_command->insert($database_client_historyinfo,"client_id,user_id,comment,timestamp","'','".$login_record[1]."','Update Category (".addslashes($_POST["name"]).")','".$time."'");
...
<div style="float:left; margin-left:240px;  margin-top:10px; margin-right:10px;"><input type="button" name="" value="Back" onclick="window.location='<?php echo $site_url; ?>/oos/update-category.php'"></div>
<div style="float:right; margin-top:10px; margin-right:10px;"><input type="submit" name="action" value="Delete Category" onclick="return deletechecked();"/></div>
...
$result = $sql_command->select($database_category_extras,"id,category_name","WHERE deleted='No' ORDER BY category_name");
