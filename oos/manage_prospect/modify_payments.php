...
if (count($_POST['payall'])>=0) {
	...
	if ($_POST['action']==="Approve Payments") {
		...
		foreach($payall as $p) {
			...
			foreach($payr as $pay) {
				...
				if ($payvalr[0]>=$totalin && $row[2]!='Paid') {	
					$sql_command->update("quotation_proformas","status='Paid'","id='".addslashes($pay[0])."'");	
					$sql_command->update("quotation_proformas","updated_timestamp='".$time."'","id='".addslashes($pay[0])."'");
					
					$sql_command->update("quotation_proforma_history",
										 "quotation_proforma_history.status='Paid'",
										 "quotation_proforma_history.invoice_id='".addslashes($pay[0])."'");				
		...
		header("Location: $site_url/oos/manage-prospect.php".$uid);
	...
	elseif ($_POST['action']==="Delete Payments") {
		...
		foreach($payall as $p) {
			...
			foreach($payr as $pay) {	
				...
				$result = $sql_command->select("quotation_proformas","status",
											   "WHERE id = '".addslashes($pay[0])."'");
				$row = $sql_command->result($result);
				...
				if ($row[0]=="Paid" && $payvalr[0]>0) { 
					$sql_command->update("quotation_proformas","status='Outstanding'","id='".addslashes($pay[0])."'");
					$sql_command->update("quotation_proformas","updated_timestamp='".$time."'","id='".addslashes($pay[0])."'");
						
					$sql_command->update("quotation_proforma_history",
										 "quotation_proforma_history.status='Outstanding'",
										 "quotation_proforma_history.invoice_id='".addslashes($pay[1])."'");				
