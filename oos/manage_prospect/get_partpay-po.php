...
<option value="Refused">Refused</option>
	...
	<div class="formlist">
	 ...
	 $result = $sql_command->select("$database_supplier_invoices_main,$database_supplier_details,quotation_details,$database_clients","$database_supplier_invoices_main.id,
	...
	$list .= "
		...
		<div class=\"formlisttr\">
			...
			<div class=\"formlisttd\">
				...
				<input type=\"hidden\" name=\"".$record[1]."-invid\" value=\"".$invidr[0]."\">
				<input type=\"hidden\" name=\"".$record[1]."-date\" value=\"".$dateline."\">
				<input type=\"hidden\" name=\"".$record[1]."-total\" value=\"".$totalin."\">
			...
			<div class=\"formlisttd\">
				<input type=\"number\" name=\"".$record[1]."-alloc\" class=\"allocate\"/>
			</div>
		</div>
...
<?php 
echo $menu_line; 
$result2 = $sql_command->select("supplier_transactions","p_id,p_amount,transaction_id,ip_add,status,cardtype,timestamp,currency",
								"WHERE p_id=".$_GET['pid']);
...
foreach($row as $record) {
	...
	$invids = $sql_command->select($database_supplier_invoices_details,"invoice_id","WHERE main_id='".$record[0]."'");
	$invidr = $sql_command->result($invids);
	...
	$list .= "
		...
		<div class=\"formlisttr\">
			...
			<div class=\"formlisttd\">
				<input type=\"number\" name=\"".$record[1]."-alloc\" class=\"allocate\" value=\"".$curr."\"/>
