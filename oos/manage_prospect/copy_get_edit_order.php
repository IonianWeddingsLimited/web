...
$package_info_result = $sql_command->select("quotation_history","id,name,cost,iw_cost,currency,d_value,d_type,status,d_","WHERE order_id='".addslashes($_GET["invoice_id"])."' and item_type='Package'");
$package_info_record = $sql_command->result($package_info_result);

$order_info_result = $sql_command->select("quotation_details","package_id,reception_id,ceremony_id","WHERE id='".addslashes($_GET["invoice_id"])."'");
...
$island_result = $sql_command->select("quotation_details,$database_packages,$database_package_info","quotation_details.package_id,
									$database_packages.island_id
									","WHERE quotation_details.id='".addslashes($_GET["invoice_id"])."' and
...
$isl_q = $sql_command->select("clients","destination","WHERE id='".addslashes($_GET["id"])."'");
$isl_r = $sql_command->result($isl_q);

$island_record[1] = ($island_record[1]==5) ?   $isl_r[0]:  $island_record[1]; 
...
$get_checked_result = $sql_command->select("quotation_history","qty,d_value,d_type,d_","WHERE order_id='".addslashes($_GET["invoice_id"])."' and type_id='".addslashes($extra_record[0])."' and item_type='Extra' and type='Extra' and (status='Pending' or status='Outstanding')");
...
$html_extra3 .= "
<div style=\"float:left; width:30px; margin:5px;\"><strong>QTY</strong></div>
<div style=\"float:left; width:300px; margin:5px;\"><strong>Item</strong></div>
<div style=\"float:left; width:50px; margin:5px;\"><strong>Net</strong></div>
<div style=\"float:left; width:50px; margin:5px;\"><strong>Gross</strong></div>
<div style=\"float:left; width:30px; margin:5px;\"><strong>VLE</strong></div>
<div style=\"float:left; width:70px; margin:5px;\"><strong>Supplier</strong></div>
<div style=\"clear:left;\"></div>";
...
$sql_q = $sql_command->SELECT("$database_package_extras","supplier_id","WHERE id = '".addslashes($extra_record[0])."'");
$sql_r = $sql_command->result($sql_q);
$current_supplier = $sql_r[0];
$supplier_result = $sql_command->select($database_supplier_details,"id,company_name","WHERE deleted='No' ORDER BY company_name");
$supplier_row = $sql_command->results($supplier_result);
$supplier_list ="";
$supplier_textarea ="";
$nr_output ="";
foreach($supplier_row as $supplier_record) {
	if($supplier_record[0] == $current_supplier) { $selectedyes = "selected=\"selected\""; } else {$selectedyes = ""; }
	$supplier_list .= "<option value=\"".stripslashes($supplier_record[0])."\" $selectedyes>".stripslashes(str_replace("'","&#39",$supplier_record[1]))."</option>";
}
...
$html_extra3 .= "
<div style=\"float:left; width:30px; margin:5px;\"><input type=\"text\" name=\"extra_qty_$extra_record[0]\" style=\"width:30px;\" value=\"$qty\"></div>
<div style=\"float:left; width:300px; margin:5px;\">".stripslashes($extra_record[1])."</div>
<div style=\"float:left; width:50px; margin:5px;\">".$curreny."&nbsp;".number_format($extra_record[3],2)."</div>
<div style=\"float:left; width:50px; margin:5px;\">".$curreny."&nbsp;".number_format($iw_cost,2)."</div>
<div style=\"float:left; width:30px; margin:5px;\"><input type=\"text\" name=\"extra_d_value_$extra_record[0]\" value=\"$d_type\" style=\"width:30px;\"></div>
<div style=\"float:left; margin:5px;\">
<select style=\"width:150px;\" name=\"extra_supplier_$extra_record[0]\">".$supplier_list."
</select>
<input type=\"hidden\" name=\"extra_csupplier_$extra_record[0]\" value=\"".$current_supplier."\" />
<input type=\"hidden\" name=\"extra_d_type_$extra_record[0]\" value =\"amount\" />
<input type=\"hidden\" name=\"extra_d_$extra_record[0]\" value =\"gross\" />
</div>
<div style=\"clear:left;\"></div>";
}
} else {
...
$html_extra4 .= "
<div style=\"float:left; width:17px; margin:5px;\"><strong>Hide </strong></div>
<div style=\"float:left; width:28px; margin:5px;\"><strong>QTY</strong></div>
<div style=\"float:left; width:295px; margin:5px;\"><strong>Item</strong></div>
<div style=\"float:left; width:50px; margin:5px;\"><strong>Net</strong></div>
<div style=\"float:left; width:50px; margin:5px;\"><strong>Gross</strong></div>
<div style=\"float:left; width:30px; margin:5px;\"><strong>VLE</strong></div>
<div style=\"float:left; width:70px; margin:5px;\"><strong>Supplier</strong></div>
...
foreach($extra_row as $extra_record) {
$hideCheck = $sql_command->count_nrow("notes","notes_id","note_primary_reference = '".addslashes($_GET['invoice_id'])."' AND note_secondary_reference = '".addslashes($extra_record[0])."' AND note_type = 'pHide' AND extra = 'Yes'");
$hideC = ($hideCheck>0) ? "checked=\"checked\"": "";

if($extra_record[2] == "Pound") { $curreny = "&pound;"; } else { $curreny = "&euro;"; }


$get_checked_result = $sql_command->select("quotation_history","qty,d_value,d_type,d_","WHERE order_id='".addslashes($_GET["invoice_id"])."' and type_id='".addslashes($extra_record[0])."' and item_type='Extra' and type='Included' and (status='Pending' or status='Outstanding')");
$get_checked_record = $sql_command->result($get_checked_result);
...
if($get_checked_record[0] != 0) {
$html_extra4 .= "
<div style=\"float:left; width:60px; margin:5px 2px;\">
<input type=\"checkbox\" name=\"include_hide_$extra_record[0]\" style=\"width:17px;\" value=\"Yes\" $hideC>
<input type=\"text\" name=\"include_qty_$extra_record[0]\" style=\"width:27px;\" value=\"$qty\"></div>
<div style=\"float:left; width:300px; margin:5px;\">".stripslashes($extra_record[1])."</div>
<div style=\"float:left; width:50px; margin:5px;\">".$curreny."&nbsp;".number_format($extra_record[3],2)."</div>
<div style=\"float:left; width:50px; margin:5px;\">".$curreny."&nbsp;".number_format($iw_cost,2)."</div>
...
<div style=\"float:left; margin:5px;\"><select style=\"width:150px;\" name=\"include_supplier_$extra_record[0]\">".$supplier_list."
</select>
<input type=\"hidden\" name=\"include_csupplier_$extra_record[0]\" value=\"".$current_supplier."\" />
<input type=\"hidden\" name=\"include_d_type_$extra_record[0]\" value=\"\" />
<input type=\"hidden\" name=\"include_d_$extra_record[0]\" value=\"\" /></div>
<div style=\"clear:left;\"></div>";
}
} else {
$html_extra4 .= "
...
<div style=\"float:left; width:50px; margin:5px;\">".$curreny."&nbsp;".number_format($iw_cost,2)."</div>
<div style=\"float:left; width:30px; margin:5px;\"><input type=\"text\" name=\"include_d_value_$extra_record[0]\" value=\"$d_type\" style=\"width:30px;\"></div>
<div style=\"float:left; margin:5px;\"><select style=\"width:150px;\" name=\"include_supplier_$extra_record[0]\">
...
$get_checked_result = $sql_command->select("quotation_history","qty,d_value,d_type,d_","WHERE order_id='".addslashes($_GET["invoice_id"])."' and type_id='".addslashes($menu_option_record[0])."' and item_type='Menu' and type='Extra' and (status='Pending' or status='Outstanding')");
$get_checked_record = $sql_command->result($get_checked_result);
...
$extra_result = $sql_command->select($database_package_extras,"id,product_name,currency,cost,iw_cost,type,notes,category_id,deleted","WHERE 
									 island_id='".addslashes($island_record[1])."' AND 
									 category_id=".addslashes($extra_cat_record[0])." AND 
									 type='Extra'
									 ORDER BY product_name");
...
if (($extra_record[8] == "Yes") and ($qty == "0")) {
...
$get_checked_result = $sql_command->select("quotation_history","qty,d_value,d_type,d_","WHERE order_id='".addslashes($_GET["invoice_id"])."' and type_id='".addslashes($service_record[0])."' and item_type='Service Fee'  and type='Extra' and (status='Pending' or status='Outstanding')");
$get_checked_record = $sql_command->result($get_checked_result);
...
$hideCheck = $sql_command->count_nrow("notes","notes_id","note_primary_reference = '".addslashes($_GET['invoice_id'])."' AND note_secondary_reference = '".addslashes($package_extra_record[0])."' AND note_type = 'pHide' AND extra = 'Yes'");
$hideC = ($hideCheck>0) ? "checked=\"checked\"": "";

if($package_extra_record[5] == "Pound") { $curreny = "&pound;"; } else { $curreny = "&euro;"; }

if($added == "No") {
$html_extra_included2 .= "<div style=\"float:left; width:60px; margin:5px 2px;\"><strong>Hide QTY</strong></div>
<div style=\"float:left; width:320px; margin:5px;\"><strong>Item</strong></div>
<div style=\"float:left; width:50px; margin:5px;\"><strong>Net</strong></div>
<div style=\"float:left; width:50px; margin:5px;\"><strong>Gross</strong></div>
<div style=\"clear:left;\"></div>";
...
$get_checked_result = $sql_command->select("quotation_history","qty,d_value,d_type,d_","WHERE order_id='".addslashes($_GET["invoice_id"])."' and type_id='".addslashes($package_extra_record[0])."' and item_type='Extra' and type='Included' and (status='Pending' or status='Outstanding')");
$get_checked_record = $sql_command->result($get_checked_result);

if(!$get_checked_record[0]) { $get_checked_record[0] = 0; }
	
$html_extra_included2 .= "
<div style=\"float:left; width:60px; margin:5px 2px;\"><input type=\"checkbox\" name=\"extra_included_hide_$package_extra_record[0]\" style=\"width:17px;\" value=\"Yes\" $hideC><input type=\"text\" name=\"extra_included_qty_$package_extra_record[0]\" style=\"width:30px;\" value=\"".stripslashes($get_checked_record[0])."\"></div>
...
foreach($menu_row as $menu_record) {
$hideCheck = $sql_command->count_nrow("notes","notes_id","note_primary_reference = '".addslashes($_GET['invoice_id'])."' AND note_secondary_reference = '".addslashes($menu_record[0])."' AND note_type = 'pHide' AND extra = 'Yes'");
$hideC = ($hideCheck>0) ? "checked=\"checked\"": "";
...
<div style=\"float:left; width:60px; margin:5px 2px;\"><strong>Hide QTY</strong></div>
<div style=\"float:left; width:320px; margin:5px;\"><strong>Item</strong></div>
<div style=\"float:left; width:50px; margin:5px;\"><strong>Net</strong></div>
<div style=\"float:left; width:50px; margin:5px;\"><strong>Gross</strong></div>
<div style=\"clear:left;\"></div>";
...
$get_checked_result = $sql_command->select("quotation_history","qty,d_value,d_type,d_","WHERE order_id='".addslashes($_GET["invoice_id"])."' and type_id='".addslashes($menu_record[0])."' and item_type='Menu' and type='Included' and (status='Pending' or status='Outstanding')");
$get_checked_record = $sql_command->result($get_checked_result);
...
$html_menu_included .= "
<div style=\"float:left; width:60px; margin:5px 2px;\"><input type=\"checkbox\" name=\"menu_included_hide_$menu_record[0]\" style=\"width:17px;\" value=\"Yes\" $hideC><input type=\"text\" name=\"menu_included_qty_$menu_record[0]\" style=\"width:30px;\" value=\"".stripslashes($get_checked_record[0])."\"></div>
...
if($added == "No") {
$html_extra_included .= "<h3 style=\"margin-top:10px; margin-bottom:10px;\">".stripslashes($package_extra_record[6])."</h3><div style=\"float:left; width:60px; margin:5px 2px;\"><strong>Hide QTY</strong></div>
<div style=\"float:left; width:320px; margin:5px;\"><strong>Item</strong></div>
<div style=\"float:left; width:50px; margin:5px;\"><strong>Net</strong></div>
<div style=\"float:left; width:50px; margin:5px;\"><strong>Gross</strong></div>
<div style=\"clear:left;\"></div>";
...
foreach($package_service_row as $package_service_record) {
	$hideCheck = $sql_command->count_nrow("notes","notes_id","note_primary_reference = '".addslashes($_GET['invoice_id'])."' AND note_secondary_reference = '".addslashes($package_service_record[0])."' AND note_type = 'pHide' AND extra = 'Yes'");
$hideC = ($hideCheck>0) ? "checked=\"checked\"": "";
...
$get_checked_result = $sql_command->select("quotation_history","qty,d_value,d_type,d_","WHERE order_id='".addslashes($_GET["invoice_id"])."' and type_id='".addslashes($package_service_record[0])."' and item_type='Service Fee' and type='Included' and (status='Pending' or status='Outstanding')");
$get_checked_record = $sql_command->result($get_checked_result);
...
<div style=\"float:left; width:60px; margin:5px 2px;\"><input type=\"checkbox\" name=\"service_included_hide_$package_service_record[0]\" style=\"width:17px;\" value=\"Yes\" $hideC><input type=\"text\" name=\"service_included_qty_$package_service_record[0]\" style=\"width:30px;\" value=\"".stripslashes($get_checked_record[0])."\"></div>
...
function addElement() {
  ...
  newdiv.innerHTML = '<div style="float:left; width:30px; margin:5px;"><input type="text" name="bespoke_extra_qty_' + num +'" style="width:20px;"/></div>'
...
function addElement2() {
  ...
  newdiv.style.width = '100%'
  newdiv.setAttribute('id',divIdName);
  newdiv.innerHTML = '<div style="float:left; width:60px; margin:5px 2px;"><input type="checkbox" name="bespoke_include_hide_' + num +'" style="width:15px;" value="Yes"/><input type="text" name="bespoke_include_qty_' + num +'" style="width:27px;margin-left:3px;"/></div>'
...
<div style=" margin:5px;">[ <a href="<?php echo $site_url; ?>/oos/manage-prospect.php?a=change-package&id=<?php echo $_POST["client_id"]; ?>&invoice_id=<?php echo $_GET["invoice_id"]; ?>">Change Package</a> ]</div>
<?php } ?>
...
<div style="float:left; width:60px; margin:5px 2px;"><input type="checkbox" name="bespoke_include_hide_1" style="width:17px;" value="Yes"/><input type="text" name="bespoke_include_qty_1" style="width:27px;margin-left:3px;"/></div>
...
<div id="add_item2" style=" padding:0; margin:0;"> </div>
...
<div style="float:left; width:30px; margin:5px;"><input type="text" name="bespoke_extra_qty_1" style="width:20px;"/></div>
...
<div id="add_item" style="padding:0; margin:0;"> </div>
...
<div style="float:left;"><p><input type="submit" name="action" value="Update Quote" onclick="return checkexchange();"></p></div>
<div style="float:left; margin-left:200px;"><p><input type="button" name="" value="Back"   onclick="window.location='<?php echo $site_url; ?>/oos/manage-prospect.php?a=history&id=<?php echo $_POST["client_id"]; ?>'"></p></div>
</form>
