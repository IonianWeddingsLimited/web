...
if ($posttype == "add") {
	...
	if($error) {
		...
		$get_template->errorHTML("Manage Prospect","Oops!","$error","Link","oos/manage-prospect.php?a=partpay&id=".$_POST["client_id"]."&pid=".$_POST["payment_id"]."&f=add");
	...
	foreach ($payinv as $p) {
		...
		if($allocate>0) { 
			...
			$result = $sql_command->select("quotation_proformas","iw_cost,type,status",
			...
			if ($payvalr[0]>=$totalin && $row[2]!='Paid' && $pstatus=='Paid') {	
				$sql_command->update("quotation_proformas","status='Paid'","id='".addslashes($invid)."'");	
				$sql_command->update("quotation_proformas","updated_timestamp='".$time."'","id='".addslashes($invid)."'");
				
				$sql_command->update("quotation_proforma_history",
									 "quotation_proforma_history.status='Paid'",
									 "quotation_proforma_history.invoice_id='".addslashes($invid)."'");				
...
//	echo count($payinv);
	...
	if($error) {
		$get_template->topHTML();
		$get_template->errorHTML("Manage Prospect","Oops!","$error","Link","oos/manage-prospect.php?a=partpay&id=".$_POST["client_id"]."&pid=".$_POST['payment_id']."&f=mod");
		$get_template->bottomHTML();	
		$sql_command->close();
	}
	...
	foreach ($payinv as $p) {
		...
		if ($payvalr[0]>=$totalin && $row[2]!='Paid' && $pstatus=='Paid' && ($allocate > 0 || $current >0)) {	
			...
			$sql_command->update("quotation_history",
								 "quotation_history.status='Paid'",
								 "quotation_history.invoice_id='".addslashes($invid)."'");				
			...
			if ($row[2]=="Paid" && ($allocate>0 or $current>0)) {	
				$pstatus = "Outstanding";
				$success = $sql_command->update("quotation_proformas","status='Outstanding'","id='".addslashes($invid)."'");
				$success = $sql_command->update("quotation_proformas","updated_timestamp=".$time,"id='".addslashes($invid)."'");
			
				$result = $sql_command->select("quotation_proformas","iw_cost,type",
											   "WHERE id = '".addslashes($invid)."'
											   AND (status != 'Paid' or status != 'Cancelled')");
				$row = $sql_command->result($result);
					
				$sql_command->update("quotation_proforma_history",
									 "quotation_proforma_history.status='Outstanding'",
									 "quotation_proforma_history.invoice_id='".addslashes($invid)."'");				
