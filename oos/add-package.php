...
if(!$_POST["cost_1"]) { $error .= "Missing Net 1<br>"; }
if(!$_POST["iw_cost_1"]) { $error .= "Missing Gross 1<br>"; }
...
$cols = "island_id,supplier_id,ceremony_id,reception_id,package_name,deleted";


$values = "'".addslashes($_POST["island_id"])."',
'".addslashes($_POST["supplier_id"])."',
'".addslashes($_POST["ceremony_id"])."',
...
$sql_command->insert($database_client_historyinfo,"client_id,user_id,comment,timestamp,package_id,island_id","'','".$login_record[1]."','Package Added','".$time."','".$maxid."','".addslashes($_POST["island_id"])."'");
...
$ceremony_list .= "<option value=\"".stripslashes($ceremony_record[0])."\">".stripslashes($ceremony_record[1])."</option>";
}
...
+ '<div style="float:left; width:60px; margin:5px;"><input type="text" name="cost_' + num +'" style="width:60px;" value="0.00" /></div>'
...
<div style="float:left; width:60px; margin:5px;"><input type="text" name="cost_1" style="width:60px;" value="0.00" /></div>
...
$nav_list2 .= "".stripslashes($level2_record[0])." - ".stripslashes($level2_record[1])."<br>";
...
$nav_list2 .= "".stripslashes($level3_record[0])." - ".stripslashes($level3_record[1])."<br>";
}
}
} 
}
