...
/*$checkinc = $sql_command->select("order_history",
								 ...
								 code,
								 exchange_rate",
								 "WHERE order_history.order_id = '".addslashes($record[17])."' 
								 AND NOT EXISTS(SELECT * FROM supplier_invoice_details WHERE supplier_invoice_details.order_id='".addslashes($record[17])."' AND supplier_invoice_details.code = order_history.code) 
								 AND EXISTS(SELECT * FROM package_extras WHERE package_extras.supplier_id = '".addslashes($record[0])."' AND package_extras.code = order_history.code)");
...
foreach ($checkin as $c) {
	...
	if ($qties > 0 || $qties < 0) {
		...
		$c[1] = str_replace("<strong>", "", $c[1]);
		$c[1] = str_replace("</strong>", "", $c[1]);
		$c[1] = str_replace("<u>", "", $c[1]);
		$c[1] = str_replace("</u>", "", $c[1]);
		$c[1] = str_replace("<i>", "", $c[1]);
		$c[1] = str_replace("</i>", "", $c[1]);
		if(eregi("<p>",$c[1])) {
			$start = strpos($c[1], '<p>');
			$end = strpos($c[1], '</p>', $start);
			$paragraph = substr($c[1], $start, $end-$start+4);	
			$paragraph = str_replace("<p>", "", $paragraph);	
			$paragraph = str_replace("</p>", "", $paragraph);	
			$paragraph = (strlen($paragraph) > 150) ? substr($paragraph, 0, 150) . '...' : $paragraph;	
		} else {	
			$paragraph = stripslashes($c[1]);	
		}	

		$paragraph = preg_replace('~[\r\n]+~', '', $paragraph);
		$paragraph = str_replace("&nbsp;", " ", $paragraph);
		$paragraph = trim(preg_replace('/\s\s+/', ' ', $paragraph));
		//$itemvalue = $c[3] * $record[16];
		...
		$pdf->SetLeftMargin('10');
		$pdf->SetFont('','',5);
		$pdf->Cell(160.05,4,$c[5].' - '.$paragraph,'LR',0,'L',true);	
		$pdf->SetFont('','',6);	
...
$result2 = $sql_command->select("$database_supplier_invoices_details",
								...
								code,
								invoice_id",
								"WHERE main_id='".addslashes($_GET["purchase_order"])."'");
$row2 = $sql_command->results($result2); 
$codecheck = array();
foreach($row2 as $record2) {
	
	/*
	SELECT $database_order_history.qty, $database_order_history.cost
	FROM $database_order_history 
	LEFT OUTER JOIN $database_supplier_invoices_details.code ON 
	$database_order_history.code = $database_supplier_invoices_details.code 
	AND $database_supplier_invoices_details.code = $database_order_history.code
	WHERE $database_order_history.order_id = 373
	AND $database_supplier_invoices_details.code = NULL
	*/
	...
	//$test = "select sum(qty),exchange_rate from $database_order_history WHERE order_id='".addslashes($record[17])."' AND code = '".addslashes($record2[5])."'";
...
$resp = $sql_command->select("supplier_payments,supplier_transactions",
					 ...
					 AND supplier_payments.main_id = '".addslashes($_GET["purchase_order"])."'");
...
if ($comments_r) {
	...
	$pdf->MultiCell(190.05, 3, "Please Note", 'LR', 'L', true);
	$pdf->Ln(2);
...
//	$bullet_list = array();
	...
	for ($i = 0; $i < $count; ++$i) {
		...
		$pdf->MultiCell(190.05, 3, str_replace($filter_p,"",$matches[0][$i]).$y, 'LR', 'L', true);
		$pdf->Ln(2);
