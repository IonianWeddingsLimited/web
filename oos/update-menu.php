...
if(!$_POST["menu_id"]) { $error .= "Missing Menu Id<br>"; }
if(!$_POST["island_id"]) { $error .= "Missing Menu Id<br>"; }
	
	
	
if($error) {
$get_template->topHTML();
$get_template->errorHTML("Delete Menu","Oops!","$error","Link","oos/update-menu.php");
$get_template->bottomHTML();
$sql_command->close();
}

$sql_command->update($database_menus,"deleted='Yes'","island_id='".addslashes($_POST["island_id"])."' and id='".addslashes($_POST["menu_id"])."'");
$sql_command->update($database_menu_options,"deleted='Yes'","menu_id='".addslashes($_POST["menu_id"])."'");
$sql_command->insert($database_client_historyinfo,"client_id,user_id,comment,timestamp,package_id,island_id","'','".$login_record[1]."','Menu Deleted (".addslashes($_POST["iw_menu_name"]).")','".$time."','','".addslashes($_POST["island_id"])."'");

$get_template->topHTML();
?>
<h1>Menu Deleted</h1>

<p>The menu has now been deleted</p>
...
$code =  $menu_option_record[0] . strtoupper($first) . strtoupper($middle) . strtoupper($last) . "M";
$sql_command->update($database_menu_options,"code='".addslashes($code)."'","menu_id='".addslashes($_POST["menu_id"])."' and id='".addslashes($menu_option_record[0])."'");
...
$cols = "menu_id,menu_name,cost,cost_iw,currency,note,timestamp,code,deleted";
...
$maxid_menu = $sql_command->maxid($database_menu_options,"id");		
...
$code =  $maxid_menu . strtoupper($first) . strtoupper($middle) . strtoupper($last) . "M";
$sql_command->update($database_menu_options,"code='".addslashes($code)."'","id='".$maxid_menu."'");
...
$sql_command->insert($database_client_historyinfo,"client_id,user_id,comment,timestamp,package_id,island_id","'','".$login_record[1]."','Menu Updated (".addslashes($_POST["iw_menu_name"]).")','".$time."','','".addslashes($_POST["island_id"])."'");
...
<p style="text-align:right;"><input type="submit" name="action" value="Delete Menu" onclick="return deletechecked();"/></p>
...
<option value="Euro" <?php if($menu_option_record[5] == "Euro") { echo "selected=\"selected\""; } ?>>Euro</option>
<option value="Pound"  <?php if($menu_option_record[5] == "Pound") { echo "selected=\"selected\""; } ?>>Pound</option>
</select></div>
...
$menu_result = $sql_command->select("$database_menus,$database_supplier_details","$database_menus.id,
									...
									$database_menus.island_id='".addslashes($_GET["island_id"])."' and 
