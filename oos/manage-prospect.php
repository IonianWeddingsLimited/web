...
if($_POST["a"] == "Continue") {
header("Location: $site_url/oos/manage-prospect.php?a=history&id=".$_POST["id"]);
$sql_command->close();
}
...
<p style=\"margin-top:10px;\">[ <a href=\"$site_url/oos/manage-prospect.php?a=history&id=".$_POST["client_id"]."\">Quote History</a> | 
																								<a href=\"$site_url/oos/manage-prospect.php?a=add-order&id=".$_POST["client_id"]."\">Add Quote</a> | 
																								<a href=\"$site_url/oos/manage-prospect.php?a=view&id=".$_POST["client_id"]."\">Update Prospect</a> | 
																								<a href=\"$site_url/oos/manage-prospect.php?a=add-note&id=".$_POST["client_id"]."\">Add Note</a> | 
																								<a href=\"$site_url/oos/manage-prospect.php?a=view-notes&id=".$_POST["client_id"]."\">View Notes</a> ] ";


if($_GET["a"] == "edit-order") {
//	switch($the_username) {
//	case "u1":
include("manage_prospect/copy_get_edit_order.php");
//	break;
	//default:
//include("manage_prospect/get_edit_order.php");
//	break;
//}
...
include("manage_prospect/get_add_note.php");

} elseif($_POST["action"] == "View Note") {
	
include("manage_prospect/get_view_note.php");


} elseif($_POST["action"] == "Add Note") {
	
include("manage_prospect/get_save_note.php");

} elseif($_POST["action"] == "Update Note") {
	
include("manage_prospect/get_save_update_note.php");

} elseif($_POST["action"] == "Delete Note") {
	
include("manage_prospect/get_delete_note.php");

} elseif ($_POST["action"] =="Update Comments") {
	include("manage_prospect/get_update_comments.php");
	
} elseif($_GET["a"] == "view-notes") {
	
include("manage_prospect/get_view_notes.php");

} elseif($_POST["action"] == "Delete Order") {
	
$sql_command->delete("quotation_details","id='".addslashes($_POST["invoice_id"])."'");
$sql_command->delete("quotation_proforma_history","order_id='".addslashes($_POST["invoice_id"])."'");
$sql_command->delete("quotation_history","order_id='".addslashes($_POST["invoice_id"])."'");
...
} elseif($_POST["action"] == "Update Quote") {


//switch($the_username) {
//	case "u1":
	include("manage_prospect/copy_get_update_order_save.php");
//	break;
//	default:
//	include("manage_prospect/get_update_order_save.php");
//	break;
//}
...
//switch($the_username) {
//	case "u1":
	include("manage_prospect/copy_get_create_invoice.php");
//	break;
//	default:
//	include("manage_prospect/get_create_invoice.php");
//	break;
//}
		
	
} elseif($_POST["action"] == "Create Proforma") {
//switch($the_username) {
//	case "u1":
include("manage_prospect/copy_get_create_invoice_save.php");
//	break;
//	default:
//include("manage_prospect/get_create_invoice_save.php");
//	break;
//}

} elseif($_GET["a"] == "view-order") {

include("manage_prospect/get_view_order.php");

} elseif($_GET["a"] == "history") {
	
include("manage_prospect/get_history.php");
	...
	if($_POST["delete"] == "Yes") {
		...
		$sql_command->delete("quotation_proformas","id='".addslashes($_POST["invoice_id"])."'");	
		$sql_command->delete("quotation_proforma_history","invoice_id='".addslashes($_POST["invoice_id"])."'");
		$sql_command->update("quotation_history","status='Outstanding'","invoice_id='".addslashes($_POST["invoice_id"])."'");
		
		if($_POST["invoice_type"] == "Deposit") {
			$sql_command->update("quotation_proforma_history","status='Cancelled'","order_id='".addslashes($_POST["order_id"])."' and item_type='Deposit'");
		}
		$status_line = "Proforma Deleted (# ".$_POST["invoice_id"].")";
	}
	else {
		$sql_command->update("quotation_proformas","status='".addslashes($_POST["status"])."'","id='".addslashes($_POST["invoice_id"])."'");
		$sql_command->update("quotation_proformas","updated_timestamp='".$time."'","id='".addslashes($_POST["invoice_id"])."'");
		
		if($_POST["status"] == "Outstanding") {
			$sql_command->update("quotation_proforma_history","status='".addslashes($_POST["status"])."'","invoice_id='".addslashes($_POST["invoice_id"])."'");
		} else {
			$sql_command->update("quotation_proforma_history","status='".addslashes($_POST["status"])."'","invoice_id='".addslashes($_POST["invoice_id"])."'");
			$sql_command->update("quotation_proforma_history","status='".addslashes($_POST["status"])."'","invoice_id='".addslashes($_POST["invoice_id"])."'");	
		}
		...
		if($_POST["invoice_type"] == "Deposit") {
			$sql_command->update("quotation_proforma_history","status='".addslashes($_POST["status"])."'","order_id='".addslashes($_POST["order_id"])."' and item_type='Deposit'");
		}
...
//switch($the_username) {
//	case "u1":
include("manage_prospect/copy_get_add_order.php");
//	break;
//	default:
//include("manage_prospect/get_add_order.php");
//	break;
//}

} elseif($_GET["a"] == "deposit") {
	
include("manage_prospect/get_deposit.php");

} elseif($_GET["a"] == "emailinv") {
	
include("manage_prospect/get_email.php");
...
include("manage_prospect/get_partpay.php");

}elseif($_POST["action"] == "Add Deposit") {
	
include("manage_prospect/get_add_deposit.php");

}
elseif($_POST["action"] == "Add Payment") {
	
include("manage_prospect/get_add_payment.php");
...
include("manage_prospect/modify_payments.php");
...
//switch($the_username) {
//	case "u1":
include("manage_prospect/copy_get_select_package.php");
//	break;
//	default:
//include("manage_prospect/get_select_package.php");
//	break;
//}
	


} elseif($_POST["action"] == "Create Quote") {
//switch($the_username) {
//	case "u1":
include("manage_prospect/copy_get_process_new_order.php");
//	break;
//	default:
//include("manage_prospect/get_process_new_order.php");
//	break;
//}
...
include("manage_prospect/get_update_client.php");

} elseif($_POST["action"] == "Update Client") {
	
include("manage_prospect/get_update_client_save.php");

} elseif($_GET["a"] == "change-package") {
	
include("manage_prospect/get_change_package.php");

} elseif($_POST["action"] == "save_change_package") {
	
include("manage_prospect/get_update_change_package.php");

} elseif($_POST["action"] == "Change Package") {

include("manage_prospect/get_save_change_package.php");
...
$result = $sql_command->select($database_clients,"id,title,firstname,lastname,destination,groom_title,groom_firstname,groom_surname,iwcuid,wedding_date","WHERE deleted='No' ORDER BY firstname,lastname");
...
$result2 = $sql_command->select("$database_clients,clients_options",
							   "$database_clients.id,
							   ...
							   $database_clients.wedding_date",
								...
								AND clients_options.client_option = 'prospect'
								ORDER BY $database_clients.firstname,$database_clients.lastname");
$row2 = $sql_command->results($result2);

foreach($row2 as $record2) {
	
$des_result = $sql_command->select($database_navigation,"id,page_name","WHERE id='".addslashes($record2[4])."'");
$des_record = $sql_command->result($des_result);

$date2 = date("d-m-Y",$record2[4]);

$list2 .= "<option value=\"".stripslashes($record2[0])."\" style=\"font-size:10px;\">".stripslashes($record2[2])." ".stripslashes($record2[3])." - ".stripslashes($record2[6])." ".stripslashes($record2[7])." ( ".stripslashes($record2[8])." ) - ".$date." - ".stripslashes($des_record[1])."</option>\n";
...
<form method="post" action="<?php echo $site_url; ?>/oos/manage-prospect.php" name="getcsvdata">
...
*/
?>
<a style="text-align:right;" width="100%" href="<?php echo $site_url; ?>/oos/add-prospect.php">Add New Prospect</a>
...
<select name="id" class="inputbox_town" size="20" style="width:700px;" onclick="this.form.submit();"><?php echo $list2; ?></select>
