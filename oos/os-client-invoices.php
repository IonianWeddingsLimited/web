...
$result = $sql_command->select("$database_customer_invoices,$database_order_details,$database_clients","$database_customer_invoices.id,
							   ...
							   $database_clients.groom_surname","WHERE 
							   ...
							   ($database_customer_invoices.status = 'Outstanding' or $database_customer_invoices.status = 'Pending' or $database_customer_invoices.status = 'Quotation')
...
<div style=\"float:left; width:60px; margin:5px;\">".$record[1]."</div>
<div style=\"float:left; width:80px; margin:5px;\">&pound; ".$record[2]."</div>
<div style=\"float:left; width:140px; margin:5px;\">".$record[5]."</div>
<div style=\"float:left; width:100px; margin:5px;\">".$record[3]."</div>
<div style=\"float:left; margin:5px;\"><a href=\"$site_url/oos/manage-client.php?a=history&id=".$record[6]."\">".stripslashes($record[7])." ".stripslashes($record[8])." ".stripslashes($record[9])." &amp;<br />".stripslashes($record[10])." ".stripslashes($record[11])." ".stripslashes($record[12])."</a></div>
...
?>
<h1>Outstanding Client Invoices</h1>

<div style="float:left; width:80px; margin:5px;"><strong>Date</strong></div>
<div style="float:left; width:60px; margin:5px;"><strong>Order #</strong></div>
...
<div style="float:left; width:100px; margin:5px;"><strong>Status</strong></div>
