...
$ceremony_result = $sql_command->select("$database_package_includes,$database_package_extras,$database_category_extras","$database_package_extras.id,
									...
									$database_package_includes.package_id='".addslashes($_POST["package_id"])."' and
...
foreach($ceremony_row as $ceremony_record) {
	if($ceremony_record[5] == "Pound") { $curreny = "&pound;"; } else { $curreny = "&euro;"; }

	if($added == "No" and $ceremony_record[8] == "Yes") {
		$html_ceremony_included .= "<h3 style=\"margin-top:10px; margin-bottom:10px;\">".stripslashes($ceremony_record[6])."</h3>";
		$added = "Yes";
	}

	if($ceremony_record[8] == "Yes") { 
		$ceremony_record[1] = ($ceremony_record[1]==0) ? $ceremony_record[7] : $ceremony_record[1];
		$html_ceremony_included .= "
		<div style=\"float:left; width:20px; margin:5px;\"><input type=\"text\" name=\"extra_included_qty_$ceremony_record[0]\" style=\"width:20px;\" value=\"".stripslashes($ceremony_record[1])."\"></div>
		<div style=\"float:left; width:380px; margin:5px;\">".stripslashes($ceremony_record[2])."</div>
		<div style=\"float:left; width:70px; margin:5px;\">$curreny ".stripslashes($ceremony_record[3])."</div>
...
$package_extra_result = $sql_command->select("$database_package_includes,$database_package_extras,$database_category_extras",
											 ...
											 and $database_package_includes.type_id=package_extras.id
										 	 and $database_package_extras.category_id=category_extras.id
											 and $database_package_extras.category_id!=59
										     and $database_package_includes.type='Extra'
											 and $database_package_extras.deleted='No'
											 and $database_category_extras.deleted='No'
											 and $database_package_includes.included='Yes'");
...
$package_service_result = $sql_command->select("$database_package_includes,$database_package_extras,$database_category_extras","$database_package_extras.id,
									...
									","WHERE $database_package_includes.package_id='".addslashes($_POST["package_id"])."' 
									 ...
									 and $database_package_extras.category_id=category_extras.id
									 and $database_package_extras.category_id!=59
									 and $database_package_includes.type='Service Fee'
									 and $database_package_extras.deleted='No'
									 and $database_category_extras.deleted='No'
									 and $database_package_includes.included='Yes'");
...
<form action="<?php echo $site_url; ?>/oos/manage-client.php" method="POST">
  ...
  <?php } ?>
  <?php if ($html_ceremony_included) { ?>
  <h2 style="margin-top:10px; margin-bottom:10px;">Ceremony Packages</h2>
  <?php } ?>
  <?php echo $html_ceremony_included; ?>
  ...
  <div style="float:right; margin-top:10px; margin-right:10px;">
    <input type="button" name="" value="Back" onclick="window.location='<?php echo $site_url; ?>/oos/manage-client.php?a=edit-order&id=<?php echo $_POST["client_id"]; ?>&invoice_id=<?php echo $_POST["invoice_id"]; ?>'">
  </div>
