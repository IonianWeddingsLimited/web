...
$get_template->errorHTML("Oops!","Missing Client or PaymentID","Link",$script);
...
?>
	...
	<div class="formlist">
	  ...
	  $result = $sql_command->select("$database_customer_invoices,$database_order_details","$database_customer_invoices.id,
							   ...
							   "WHERE $database_customer_invoices.order_id=$database_order_details.id AND $database_order_details.client_id='".$_GET["id"]."' ORDER BY $database_customer_invoices.id DESC");
							   //"WHERE $database_customer_invoices.order_id=$database_order_details.id AND $database_order_details.client_id='".$_GET["id"]."' AND $database_customer_invoices.status != 'Paid' ORDER BY $database_customer_invoices.id DESC");
...
<div class="formlist">
  ...
  <?php $result = $sql_command->select("$database_customer_invoices,$database_order_details","$database_customer_invoices.id,
							   $database_customer_invoices.iw_cost,
							   $database_customer_invoices.status,
							   $database_customer_invoices.timestamp,
							   $database_customer_invoices.type,
							   ...
							   $database_customer_invoices.order_id=$database_order_details.id AND $database_order_details.client_id='".$_GET["id"]."' ORDER BY $database_customer_invoices.id DESC");
$row = $sql_command->results($result);
$list = "";
$total_prev	= 0;
foreach($row as $record) {
