...
foreach($menu_row as $menu_record) {
	...
	foreach($package_extra_row as $package_extra_record) {
		...
		if($package_extra_record[5] == "Pound") {
			$curreny = "&pound;";
		} else {
			$curreny = "&euro;";
		}
		...
		if($package_extra_record[8] == "Yes") {
			...
			<div style=\"float:left; width:56px; margin:5px;\"><input type=\"checkbox\" name=\"extra_included_hide_$package_extra_record[0]\" style=\"width:20px;\" value=\"Yes\"><input type=\"text\" name=\"extra_included_qty_$package_extra_record[0]\" style=\"width:20px;\" value=\"".stripslashes($package_extra_record[7])."\"></div>
...
foreach($menu_row as $menu_record) {
	...
	if($menu_record[11] == "Yes") { 
		$html_menu_included .= "
		<div style=\"float:left; width:56px; margin:5px;\"><input type=\"checkbox\" name=\"menu_included_hide_$menu_record[0]\" style=\"width:20px;\" value=\"Yes\"><input type=\"text\" name=\"menu_included_qty_$menu_record[0]\" style=\"width:20px;\" value=\"".stripslashes($menu_record[10])."\"></div>
...
foreach($package_service_row as $package_service_record) {
	...
	$html_service_included .= "		
		<div style=\"float:left; width:56px; margin:5px 2px;\"><input type=\"checkbox\" name=\"service_included_hide_$package_extra_record[0]\" style=\"width:20px;\" value=\"Yes\"><input type=\"text\" name=\"service_included_qty_$package_extra_record[0]\" style=\"width:20px;\" value=\"".stripslashes($package_service_record[7])."\"></div>
...
foreach($supplier_row as $supplier_record) {
	...
	$supplier_textarea .= ($si>1) ? "": "<div id=\"textarea_".$supplier_record[0]."\" style=\"float:left; margin:5px;\"><textarea style=\"width:400px; height:75px;\" name=\"cSupplier_".$supplier_record[0]."\" id=\"cSupplier\" placeholder=\"Please enter your comments here.\"></textarea></div>"; 
	$si++;
}
...
function addElement2() {
  ...
  var divIdName = 'my'+num+'Div';
  newdiv.setAttribute('id',divIdName);
  newdiv.innerHTML = '<div style="float:left; width:60px; margin:5px 2px;"><input type="checkbox" name="bespoke_include_hide_' + num + '" style="width:17px;" value="Yes"/><input type="text" name="bespoke_include_qty_' + num + '" style="width:20px;"/></div>'
...
<?php echo $menu_line; ?>
<h2>Add Order > Select Package > Add to Package s</h2>
...
<div style="clear:left;"></div>
<div style="float:left; width:60px; margin:5px 2px;"><input type="checkbox" name="bespoke_include_hide_1" style="width:17px;" value="Yes"/><input type="text" name="bespoke_include_qty_1" style="width:20px;"/></div>
...
<div style="clear:left;"></div>
<div id="add_item2" style="width:100%; padding:0; margin:0;"> </div>
