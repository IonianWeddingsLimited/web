...
$get_template->errorHTML("Manage Client","Oops!","$error","Link","oos/manage-client.php?a=create-invoice&id=".$_POST["client_id"]."&invoice_id=".$_POST["order_id"]);
...
$notes_q=$sql_command->SELECT("notes","notes.note,notes.extra,notes.notes_id,notes.note_secondary_reference,notes.timestamp,userid","WHERE notes.note_primary_reference = '".addslashes($_POST['order_id'])."' AND notes.note_type = 'InvoiceComment' AND extra!='Cancelled'");
$notes_r = $sql_command->results($notes_q);
foreach ($notes_r as $nr) {
...
header("Location: $site_url/oos/manage-client.php?a=create-invoice&id=".$_POST["client_id"]."&invoice_id=".$_POST["order_id"]."&cu=true");
