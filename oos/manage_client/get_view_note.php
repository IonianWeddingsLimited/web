...
if(!$_POST["note_id"]) {
$get_template->topHTML();
$get_template->errorHTML("Manage Client","Oops!","Please select a note","Link","oos/manage-client.php?a=view-notes&id=".$_POST["client_id"]);
$get_template->bottomHTML();
$sql_command->close();
}
...
<div style="float:left; margin-top:10px; margin-left:200px;"><input type="button" name="" value="Back" onclick="window.location='<?php echo $site_url; ?>/oos/manage-client.php?a=view-notes&id=<?php echo $_POST["client_id"]; ?>'"></div>
</form>
<form action="<?php echo $site_url; ?>/oos/manage-client.php" method="post">
<input type="hidden" name="client_id" value="<?php echo $_POST["client_id"]; ?>" />
<input type="hidden" name="note_id" value="<?php echo $_POST["note_id"]; ?>" />
