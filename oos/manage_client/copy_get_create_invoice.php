...
foreach ($notes_r as $nr) {
	...
	<div class=\"textarea\" style=\"float:left; margin:5px;\">
		<textarea name=\"ucSupplier_".$nr[2]."\" id=\"cSupplier".$nr[2]."\" placeholder=\"Please enter your comments here.\" >".$nr[0]."</textarea>
		<script type=\"text/javascript\">
			CKEDITOR.replace( 'cSupplier".$nr[2]."',
			{
				skin : 'kama',
				toolbar : 'Bullet',
				width: 400,
...
$notes_q=$sql_command->SELECT("notes","notes.note,notes.extra,notes.notes_id,notes.note_secondary_reference,notes.timestamp,userid","WHERE notes.note_primary_reference = '".addslashes($_GET['invoice_id'])."' AND notes.note_type = 'InvoiceComment' and notes.extra != 'Cancelled'");
$notes_r = $sql_command->results($notes_q);
foreach ($notes_r as $nr) {
	$check_inv = $sql_command->count_nrow("order_invoice_history","id","status!='Cancelled' AND invoice_id = '".addslashes($nr[3])."'");
//	if ($check_inv>0) {
...
//		<input type=\"checkbox\" value=\"Yes\" $checkedc /> : Show on invoice.<br /><br />
		$cc_output .= "<div style=\"float:left; width:160px; margin:5px;\"><input type=\"hidden\" name=\"clientsCShow\" id=\"clientsCShow\" value=\"Yes\" />Invoice ID:  ".$nr[3]."<br />Posted:   ".$dd."/".$mm."/".$yy."<br />User: ".$userID."<br /><br />Delete Comment: <input type=\"checkbox\" name=\"delete[]\" id=\"dComment\" value=\"".$nr[2]."\" /></div><div style=\" min-height:90px;\" id=\"textarea_wrap\"><div class=\"textarea\" style=\"float:left; margin:5px;\">
		<textarea name=\"ucClient_".$nr[2]."\" id=\"cClient".$nr[2]."\" placeholder=\"Please enter your comments here.\" >".$nr[0]."</textarea>
		<script type=\"text/javascript\">
			CKEDITOR.replace( 'cClient".$nr[2]."',
			{
				skin : 'kama',
				toolbar : 'Bullet',
				width: 400,
...
$supplier_orderid = $sql_command->SELECT("order_history","type_id,item_type","WHERE order_history.order_id = '".addslashes($_GET['invoice_id'])."' GROUP BY type_id");
...
foreach($orderid_row as $or) {
	...
	//$debug_i = "select supplier_details.id,supplier_details.company_name from package_extras,supplier_details,order_history WHERE order_history.order_id = '".addslashes($_GET['invoice_id'])."' AND order_history.type_id = package_extras.id AND package_extras.supplier_id = supplier_details.id GROUP BY supplier_details.id ORDER BY supplier_details.company_name";
...
<form action="<?php echo $site_url; ?>/oos/manage-client.php" method="post" id="myform">
	...
	<div style="float:left; width:30px; margin:5px 1px;">
		<input type="checkbox" name="id_<?php echo stripslashes($package_info_record[0]); ?>" value="Yes">
	</div>
	<?php } else { ?>
	...
	<div style="float:left; width:300px; margin:5px;"><?php echo stripslashes($package_info_record[1]); ?>
		<?php if($includes) { ?>
		<br />
		<strong>Includes</strong><br />
		<?php } ?>
		<?php echo $includes; ?></div>
	...
	<div style="clear:left;"></div>
	<?php if($html_extra2) { ?>
	<h2 style="margin-top:10px; margin-bottom:10px;">Category Extras</h2>
	<div style="float:left; width:30px; margin:5px 1px;"><strong>Inc</strong></div>
	<div style="clear:left;"></div>
	<?php } ?>
	...
	<div style="clear:left;"></div>
	<?php } ?>
	<?php echo $html_service; ?> <br />
	<div style="float:left; width:40px; margin:10px 5px 5px 5px;">
	...
	<div style="float:left; margin:5px;">
		<input type="text" name="exchange_rate" id="exchange_rate" value="<?php echo $orderdetail_info_record[0]; ?>">
	</div>
	<div style="clear:left;"></div>
	<div style="float:left; width:160px; margin:5px;"><strong>GBP / Euros</strong></div>
	<div style="float:left; margin:5px;">
		<select name="cbased" id="cbased">
			<option value="uk" selected>GBP</option>
			<option value="continental">Euro</option>
		</select>
	</div>
	<div style="clear:left;"></div>
	<?php 
//<input type=\"checkbox\" name=\"clientsCShow\" id=\"clientsCShow\" value=\"Yes\" checked/> : Show on Invoice</div>
echo "<div style=\"float:left; width:160px; margin:5px;\"><strong>Add new client comment</strong><br />
<input type=\"hidden\" name=\"clientsCShow\" id=\"clientsCShow\" value=\"Yes\" /></div>
<div style=\"float:left; margin:5px;\">
<textarea name=\"cClient\" id=\"cClient\" placeholder=\"Please enter your comments here.\"></textarea>
<script type=\"text/javascript\">
	CKEDITOR.replace( 'cClient',
	{
		skin : 'kama',
		toolbar : 'Bullet',
		width: 400,
...
?>
	<div style="float:left; margin-top:10px;">
		<input type="submit" name="action" value="Create Invoice" onclick="return checkexchange();"/>
	</div>
	<div style="float:right; margin-top:10px; margin-right:10px;">
		<input type="button" name="" value="Back" onclick="window.location='<?php echo $site_url; ?>/oos/manage-client.php?a=history&id=<?php echo $_POST["client_id"]; ?>'">
	</div>
	<div style="clear:both;"></div>
</form>
<br />
<form action="<?php echo $site_url; ?>/oos/manage-client.php" method="post" id="comments" name="updatecomments">
...
$get_inv = $sql_command->select("order_invoice_history","DISTINCT invoice_id","WHERE status!='Cancelled' AND order_id = '".addslashes($_GET['invoice_id'])."'");
$get_res = $sql_command->results($get_inv);
$inv = 0;
foreach ($get_res as $gr) {
	$c_inv = $sql_command->count_nrow("notes","notes_id","note_secondary_reference='".addslashes($gr[0])."' and note_type='InvoiceComment' and extra != 'Cancelled'");
...
//print_r($test_q);
	...
	echo "<div style=\"float:left; width:160px; margin:5px;\"><strong>Add comment to existing invoice: </strong><br />
	Invoice ID: ".$i_select."<br /><input type=\"hidden\" name=\"clientsCShow\" id=\"clientsCShow\" value=\"Yes\" /></div>
	<div style=\"float:left; margin:5px;\">
		<textarea name=\"aClient\" id=\"aClient\" placeholder=\"Please enter your comments here.\"></textarea>
		<script type=\"text/javascript\">
			CKEDITOR.replace( 'aClient',
			{
				skin : 'kama',
				toolbar : 'Bullet',
				width: 400,
	...
	<div id=\"textarea\" style=\"float:left; margin:5px;\">
		<textarea name=\"aSupplier\" id=\"aSupplier\" placeholder=\"Please enter your comments here.\"></textarea>
		<script type=\"text/javascript\">
			CKEDITOR.replace( 'aSupplier',
			{
				skin : 'kama',
				toolbar : 'Bullet',
				width: 400,
...
echo "<center><input type=\"button\" name=\"Back\" value=\"Back\" onclick=\"window.location='".$site_url."/oos/manage-client.php?a=history&id=".$_POST["client_id"]."'\" /><input type=\"submit\" name=\"action\" value=\"Update Comments\" /></center>";
